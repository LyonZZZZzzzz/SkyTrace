...
- iOS 与 macOS 时间控制菜单新增“1 秒/秒”实时播放档。
- 默认播放速度继续保持“1 天/秒”，选择档位后仍由播放/暂停按钮控制。
- 播放推进统一按“天/秒 × 已过真实秒数 × 86,400”换算。
- 修复此前天、小时和分钟档与菜单标签不一致的问题。
- 将播放推进提取为可测试方法，新增实时推进、60 秒推进和上限回绕测试。
- 版本更新为 1.2.10（build 13）。
